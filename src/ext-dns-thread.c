...
#define LOG_DOMAIN "ext-dns-thread"

/** 
 * \defgroup ext_dns_thread ext-DNS Thread: Portable threading API for ext-dns
 */
 ...
 */
typedef struct _extDnsThreadData {
 ...
 * for the thread.
 */
static unsigned __stdcall ext_dns_thread_proxy (axlPointer _data) 
{
	extDnsThreadData * data = (extDnsThreadData *) _data;
 ...
 */
axl_bool  ext_dns_thread_create_internal (extDnsThread      * thread_def,
					 extDnsThreadFunc    func, 
					 axlPointer          user_data,
					 va_list             args)
{
	extDnsThreadConf   conf;
	/* default configuration for joinable state, axl_true */
	axl_bool           joinable = axl_true;
#if defined(AXL_OS_UNIX)
	pthread_attr_t     attr;
#elif defined(AXL_OS_WIN32)
	extDnsThreadData * data;
	/* unsigned           thread_id; */
#endif
	...
	data                 = axl_new(extDnsThreadData, 1);
	EXT_DNS_CHECK_REF (data, axl_false);
	...
	if (pthread_attr_setdetachstate (&attr, joinable ? PTHREAD_CREATE_JOINABLE : PTHREAD_CREATE_DETACHED) != 0) {
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "failed to configure detached state for the thread (pthread_attr_setdetachstate system call failed)"); */
	...
	if (pthread_create (thread_def,
			    ...
			    func, user_data) != 0) {
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to create the new thread (pthread_create system call failed)");*/
		pthread_attr_destroy(&attr);
		return axl_false;
	} /* end if */
 ...
 * returned.
 */
axl_bool  ext_dns_thread_destroy_internal (extDnsThread * thread_def, axl_bool  free_data)
{
#if defined(AXL_OS_WIN32)
	...
	default:
	case WAIT_OBJECT_0:
		/* ext_dns_log (EXT_DNS_LEVEL_DEBUG, "thread %p stopped", thread_def->handle); */
		break;
	case WAIT_ABANDONED:
		/* ext_dns_log (EXT_DNS_LEVEL_DEBUG, "unable to stop thread %p, wait_abandoned", thread_def->handle); */
		break;

	case WAIT_TIMEOUT:
		/* ext_dns_log (EXT_DNS_LEVEL_DEBUG, "unable to stop thread %p, wait_timeout", thread_def); */
		break;
	}
	...
	default:
	case 0:
		/* ext_dns_log (EXT_DNS_LEVEL_DEBUG, "thread %p stopped with status %p", thread_def, status);  */
		break;

	case EINVAL:
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to stop non-joinable thread %p", thread_def);  */
		break;
		
	case ESRCH:
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to stop invalid thread %p", thread_def);  */
		break;

	case EDEADLK:
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to stop thread %p, deadlock detected", thread_def);  */
 ...
 * pointers.
 *
 * They are initialised to use the default extDns functions. If the
 ...
 */
extDnsThreadCreateFunc  __ext_dns_thread_create  = ext_dns_thread_create_internal;
extDnsThreadDestroyFunc __ext_dns_thread_destroy = ext_dns_thread_destroy_internal;
                                ...
                                ...)
{
	...
	/* create the thread */
	result = __ext_dns_thread_create (thread_def, func, user_data, args);
 ...
 * returned.
 */
axl_bool  ext_dns_thread_destroy (extDnsThread * thread_def, axl_bool  free_data)
{
	return __ext_dns_thread_destroy (thread_def, free_data);
}

/** 
 * @brief Allows to specify the function extDns library will call to create a
 * new thread.
 *
 * If the user does not have reason to change the default thread
 * creation mechanism this function can be ignored.
 *
 * NOTE: The thread mechanism functions (\ref ext_dns_thread_set_create
 * and \ref ext_dns_thread_set_destroy) must be set before any other
 * extDns API calls are made. Changing the thread mechanism functions
 * while extDns is running will most likely lead to memory corruption
 * or program crashes.
 *
 ...
 * handler.
 *
 * @see ext_dns_thread_set_destroy
 */
void ext_dns_thread_set_create (extDnsThreadCreateFunc create_fn)
{
	if (NULL != create_fn) 
		__ext_dns_thread_create = create_fn;
	else
		__ext_dns_thread_create = ext_dns_thread_create_internal;

	return;
}

/** 
 * @brief Allows to specify the function extDns library will call to
 ...
 * handler.
 *
 * @see ext_dns_thread_set_create
 */
void ext_dns_thread_set_destroy (extDnsThreadDestroyFunc destroy_fn)
{
	if (NULL != destroy_fn) 
		__ext_dns_thread_destroy = destroy_fn;
	else 
		__ext_dns_thread_destroy = ext_dns_thread_destroy_internal;
}
...
axl_bool  ext_dns_mutex_create  (extDnsMutex       * mutex_def)
{
	...
	if (pthread_mutex_init (mutex_def, NULL) != 0) {
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to create mutex (system call pthread_mutex_init have failed)"); */
		return axl_false;
	} /* end if */
#endif
	/* mutex created */
...
axl_bool  ext_dns_mutex_destroy (extDnsMutex       * mutex_def)
{
	...
	if (pthread_mutex_destroy (mutex_def) != 0) {
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to destroy the mutex (system call pthread_mutex_destroy have failed)"); */
		return axl_false;
	} /* end if */
...
} /* end ext_dns_mutex_destroy */

/** 
 * @brief Locks the given mutex. If the mutex is currently unlocked,
 * it becomes locked and owned by the calling thread, and
 * \ref ext_dns_mutex_lock returns immediately. If the mutex is already
 * locked by another thread, \ref ext_dns_mutex_lock suspends the calling
 ...
 * ext_dns_mutex_lock and \ref ext_dns_mutex_unlock from the same
 ...
 * not called to ext_dns_mutex_lock cannot do a ext_dns_mutex_unlock
...
void ext_dns_mutex_lock    (extDnsMutex       * mutex_def)
{
	...
	if (pthread_mutex_lock (mutex_def) != 0) {
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to lock the mutex (system call pthread_mutex_lock have failed)"); */
		return;
	} /* end if */
#endif
	/* mutex locked */
...
void ext_dns_mutex_unlock  (extDnsMutex       * mutex_def)
{
	...
	if (pthread_mutex_unlock (mutex_def) != 0) {
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to unlock the mutex (system call pthread_mutex_unlock have failed)"); */
		return;
	} /* end if */
#endif
	/* mutex unlocked */
...
axl_bool  ext_dns_cond_create    (extDnsCond        * cond)
{
	...
	if (pthread_cond_init (cond, NULL) != 0) {
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to init conditional variable (system call pthread_cond_init have failed)"); */
		return axl_false;
	} /* end if */
#endif
	/* cond created */
...
void ext_dns_cond_signal    (extDnsCond        * cond)
{
	...
	if (pthread_cond_signal (cond) != 0) {
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to signal conditional variable (system call pthread_cond_signal have failed)"); */
		return;
	} /* end if */
#endif
	/* signal ended */
...
void ext_dns_cond_broadcast (extDnsCond        * cond)
{
	...
	if (pthread_cond_broadcast (cond) != 0) {
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to signal conditional variable (system call pthread_cond_broadcast have failed)"); */
		return;
	} /* end if */
#endif
	/* broadcast ended */
...
#if defined(AXL_OS_WIN32)
/**
 * @internal Implementation to support ext_dns_cond_wait and
 * ext_dns_cond_timedwait under windows.
 */
axl_bool  __ext_dns_cond_common_wait_win32 (extDnsCond * cond, extDnsMutex * mutex, 
					   int milliseconds, axl_bool  wait_infinite)
{
	 ...
	 * semaphore until \ref ext_dns_cond_signal or \ref
	 * ext_dns_cond_broadcast> are called by another thread. */
	if (wait_infinite)
 ...
 * ext_dns_mutex_unlock) and waits for the condition variable cond to
 * be signaled. 
 * 
 ...
 * by the calling thread on entrance to \ref ext_dns_cond_wait.  Before
 * returning to the calling thread, \ref ext_dns_cond_wait re-acquires
 * mutex (as per \ref ext_dns_mutex_lock).
 * 
				 ...
				 extDnsMutex       * mutex)
{
	...
	return __ext_dns_cond_common_wait_win32 (cond, mutex, 0, axl_true);

#elif defined(AXL_OS_UNIX)
	/* wait for the condition */
	if (pthread_cond_wait (cond, mutex) != 0) {
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to wait on conditional variable (system call pthread_cond_wait have failed)"); */
		return axl_false;
	} /* end if */
#endif
	/* wait ended */
				 ...
				 long                microseconds)
{
	...
	__ext_dns_cond_common_wait_win32 (cond, mutex, microseconds / 1000, axl_false);
	...
	/* ext_dns_log (EXT_DNS_LEVEL_DEBUG, "waiting from: %d.%d", timeout.tv_sec, timeout.tv_nsec);*/
	...
	/* ext_dns_log (EXT_DNS_LEVEL_DEBUG, "to (microseconds=%ld): %d.%d", 
...
void ext_dns_cond_destroy   (extDnsCond        * cond)
{
	...
	if (pthread_cond_destroy (cond) != 0) {
		/* ext_dns_log (EXT_DNS_LEVEL_CRITICAL, "unable to destroy conditional variable (system call pthread_cond_destroy have failed)");*/
		return;
	} /* end if */
#endif
	/* wait ended */
 ...
 * 
 *  - \ref ext_dns_async_queue_push
 *  - \ref ext_dns_async_queue_pop
 *  - \ref ext_dns_async_queue_timedpop
 *
 * You can increase the reference counting by using \ref
 * ext_dns_async_queue_ref. If the reference count reaches zero value,
 * the queue is deallocated.
 *
 * A particular useful function is \ref ext_dns_async_queue_length
 ...
 * ext_dns_async_queue_unref. Note reference returned must be checked
 * to be not NULL (caused by memory allocation error).
 */
extDnsAsyncQueue * ext_dns_async_queue_new       (void)
{
	extDnsAsyncQueue * result;

	/* create the node */
	result            = axl_new (extDnsAsyncQueue, 1);
	EXT_DNS_CHECK_REF (result, NULL);

	/* init list of stored items */
	result->data      = axl_list_new (axl_list_always_return_1, NULL);
	EXT_DNS_CHECK_REF2 (result->data, NULL, result, axl_free);

	/* init mutex and conditional variable */
	ext_dns_mutex_create (&result->mutex);
	ext_dns_cond_create  (&result->cond);
						 ...
						 axlPointer         data)
{
	...
	/* get the mutex */
	ext_dns_mutex_lock (&queue->mutex);
	...
	if (queue->waiters > 0)
		ext_dns_cond_signal (&queue->cond);

	/* unlock the mutex */
	ext_dns_mutex_unlock (&queue->mutex);
 ...
 * ext_dns_async_queue_lock and \ref ext_dns_async_queue_unlock.
 * 
 * @param queue The queue where data will be pushed.
 *
 * @param data A reference to the data to be pushed. It is not allowed
 ...
 * ext_dns_async_queue_pop to receive this reference). This function
 * performs the same as \ref ext_dns_async_queue_push but skiping all
...
axlPointer         ext_dns_async_queue_pop       (extDnsAsyncQueue * queue)
{
	...
	while (axl_list_length (queue->data) == 0)
		EXT_DNS_COND_WAIT (&queue->cond, &queue->mutex);
						 ...
						 long               microseconds)
{
	...
	wait_again:
#endif
		/* check if data is available */
		ext_dns_cond_timedwait (&queue->cond, &queue->mutex, microseconds);
		...
		if (axl_list_length (queue->data) == 0) {
			...
			/* substract and check */
			ext_dns_timeval_substract (&now, &stamp, &diff);
...
void               ext_dns_async_queue_unref     (extDnsAsyncQueue * queue)
{
	...
	if (queue->reference == 0) {
		...
		ext_dns_cond_destroy (&queue->cond);
		
		/* unlock the mutex */
		ext_dns_mutex_unlock (&queue->mutex);

		/* destroy the mutex */
		ext_dns_mutex_destroy (&queue->mutex);
 ...
 * checking queue references. This is currently used by ext_dns
...
{
	extDnsAsyncQueue * _queue = (*queue);

	v_return_if_fail (_queue);

	/* get the mutex */
	ext_dns_mutex_lock (&_queue->mutex);
	...
	if (_queue->reference == 0) {
		...
		/* free the conditional var */
		ext_dns_cond_destroy (&_queue->cond);
		
		/* unlock the mutex */
		ext_dns_mutex_unlock (&_queue->mutex);

		/* destroy the mutex */
		ext_dns_mutex_destroy (&_queue->mutex);
 ...
 * ext_dns_async_queue_unlocked_push. Call to ext_dns_async_queue_push
 * will lock the caller forever until a call to
 * ext_dns_async_queue_unlock is done.
 *
 * @param queue The queue to lock.
 * 
 * NOTE: To produce portable code, the thread calling to this function
 * must also call to \ref ext_dns_async_queue_unlock. It is not
 * supported by Microsoft Windows platforms to do a call to \ref
 * ext_dns_async_queue_unlock from a different thread that issue the
 * call to \ref ext_dns_async_queue_lock.
 */
