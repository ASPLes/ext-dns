...
#define LOG_DOMAIN "ext_dns-errno"
...
void    ext_dns_errno_show_error        (extDnsCtx * ctx, int  __errno)
{
	...
	switch (errno) {
	case WSAEINTR:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEINTR);
	case WSAEACCES:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEACCES);
	case WSAEFAULT:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEFAULT);
	case WSAEINVAL:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEINVAL);
	case WSAEMFILE:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEFAULT);
	case WSAEWOULDBLOCK:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEWOULDBLOCK);
	case WSAEINPROGRESS:        
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEINPROGRESS);
	case WSAEALREADY:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEALREADY);           
	case WSAENOTSOCK:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAENOTSOCK);
	case WSAEDESTADDRREQ:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEDESTADDRREQ);
	case WSAEMSGSIZE:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEMSGSIZE);           
	case WSAEPROTOTYPE:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEPROTOTYPE);
	case WSAENOPROTOOPT:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAENOPROTOOPT);
	case WSAEPROTONOSUPPORT:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEPROTONOSUPPORT);
	case WSAESOCKTNOSUPPORT:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAESOCKTNOSUPPORT);
	case WSAEOPNOTSUPP:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEOPNOTSUPP);
	case WSAEPFNOSUPPORT:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEPFNOSUPPORT);
	case WSAEAFNOSUPPORT:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEAFNOSUPPORT);
	case WSAEADDRINUSE:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEADDRINUSE);
	case WSAEADDRNOTAVAIL:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEADDRNOTAVAIL);
	case WSAENETDOWN:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAENETDOWN);
	case WSAENETUNREACH:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAENETUNREACH);
	case WSAENETRESET:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAENETRESET);
	case WSAECONNABORTED:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAECONNABORTED);
	case WSAECONNRESET:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAECONNRESET);
	case WSAENOBUFS:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAENOBUFS);
	case WSAEISCONN:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEISCONN);
	case WSAENOTCONN:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAENOTCONN);
	case WSAESHUTDOWN:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAESHUTDOWN);
	case WSAETIMEDOUT:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAETIMEDOUT);
	case WSAECONNREFUSED:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAECONNREFUSED);
	case WSAEHOSTDOWN:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEHOSTDOWN);
	case WSAEHOSTUNREACH:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEHOSTUNREACH);
	case WSAEPROCLIM:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEPROCLIM);
	case WSASYSNOTREADY:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSASYSNOTREADY);
	case WSAVERNOTSUPPORTED:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAVERNOTSUPPORTED);
	case WSANOTINITIALISED:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSANOTINITIALISED);
	case WSAEDISCON:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAEDISCON);
	case WSATYPE_NOT_FOUND:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSATYPE_NOT_FOUND);     
	case WSAHOST_NOT_FOUND:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSAHOST_NOT_FOUND);
	case WSATRY_AGAIN:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSATRY_AGAIN);
	case WSANO_RECOVERY:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSANO_RECOVERY);
	case WSANO_DATA:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSANO_DATA);            
	case WSA_INVALID_HANDLE:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSA_INVALID_HANDLE);
	case WSA_INVALID_PARAMETER:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSA_INVALID_PARAMETER);
	case WSA_IO_INCOMPLETE:     
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSA_IO_INCOMPLETE);
	case WSA_IO_PENDING:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSA_IO_PENDING);
	case WSA_NOT_ENOUGH_MEMORY:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSA_NOT_ENOUGH_MEMORY);
	case WSA_OPERATION_ABORTED:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSA_OPERATION_ABORTED);
	case WSASYSCALLFAILURE:
		ext_dns_log (EXT_DNS_LEVEL_CRITICAL,  ERRNO_WSASYSCALLFAILURE);
	}
#elif defined (AXL_OS_UNIX)
	/* show the last error */
	ext_dns_log (EXT_DNS_LEVEL_CRITICAL, strerror (errno));
#endif
	return;
}
...
{
	ext_dns_errno_show_error (ctx, errno);
...
{
	return (errno == 0) ? NULL : ext_dns_errno_get_error (errno);
}
